...
class SelectTopicViewModel extends GetxController {
  ...
  var isFlagSelected = true.obs;
  var isRandomQuestionSelected = true.obs;
  ...
  void getToTestDetailsScreen() {
    if (selectedTopicList.isNotEmpty == true) {
      Get.toNamed(
        AppRoutes.testDetailsScreen,
      );
    } else {
      Utils.toastMessage('Select a Topic please');
    }
  ...
  void flagQuestion() {
    isFlagSelected.value = !isFlagSelected.value;
    update();
  }

  void randomQuestionSelection() {
    isRandomQuestionSelected.value = !isRandomQuestionSelected.value;
    update();
  }
}
